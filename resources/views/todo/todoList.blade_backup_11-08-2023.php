...
<body>
	...
	<script type="text/javascript">
		...
		function loadAllData(e) {
			...
			.then(dataResult => {
				...
				dataResult.forEach(function(data){
					...
					appendTask.innerHTML += '<tr id="'+task_id+'"><td><input data-task_id = "'+task_id+'" data-created_at = "'+created_at+'" type="checkbox" class="status_data" data-name="'+task_title+'" name="status" value="'+status+'" ' + (status == 1 ? 'checked' : '') + '></td><td><span class="task_title_data">'+task_title+'</span></td><td>'+created_at+'</td></tr>';
				});
				setTimeout( function() {
		...
		function showView(view) {
			// console.log("Array.from(task_data)", Array.from(task_data));
			loadAllData();
			setTimeout(() => {
				var task_data = document.getElementsByClassName("status_data");
				var original_task_data = Array.from(task_data);
				// console.log("task_list_data", original_task_data.filter((item) => item.value === "1"));
				// Filter the task_data based on the view
				// if (view == 'completed') {
				// 	task_data = Array.from(original_task_data.filter((item) => item.value === "1"));
				// } else if (view == 'remaining') {
				// 	task_data = Array.from(original_task_data.filter((item) => item.value === "0"));
				// }

				var completedTasks = original_task_data.filter((item) => item.value === "1");
				var remainingTasks = original_task_data.filter((item) => item.value === "0");
				let appendTask = document.getElementById("task_data_row"); 
				var newCompletedTasks = [];
				var newRemainingTasks = [];
				var allTasks = [];
				...
				if (view == 'completed') {
					...
					/* completedTasks.forEach(element => {
						// newCompletedTasks.push(element.getAttribute('data-name'));
					...
					// appendTask.innerHTML = newCompletedTasks;
				} else if (view == 'remaining') {
					/* remainingTasks.forEach(element => {
						newRemainingTasks.push(element.getAttribute('data-name'));
					});
					appendTask.innerHTML = newRemainingTasks; */
					appendTask.innerHTML = "";
					remainingTasks.forEach(function(element){
						let task_id = element.getAttribute('data-task_id');
						let task_title = element.getAttribute('data-name');
						let status = element.value;
						let created_at = element.getAttribute('data-created_at');
					...
					/* original_task_data.forEach(element => {
						allTasks.push(element.getAttribute('data-name'));
					});
					appendTask.innerHTML = allTasks; */
					appendTask.innerHTML = "";
					original_task_data.forEach(function(element){
						let task_id = element.getAttribute('data-task_id');
						let task_title = element.getAttribute('data-name');
						let status = element.value;
						let created_at = element.getAttribute('data-created_at');
